...
**Two-GPU parallelism (26 Sep).** Only the cross-encoder used both T4s at first (via `DataParallel`); the kNN search and XGBoost used GPU 0 only. Now the kNN shares query chunks across GPUs, XGBoost trains its independent fold models concurrently (one per GPU) and predicts on the GPU, and the two cross-fitted halves of the cross-encoder run as one process per GPU. None of this changes results (kNN and fold training are verified identical to single-GPU runs; CE parallel and sequential scores are identical with a fixed stand-in model). Each path falls back to one GPU on failure, `run.max_gpus=1` disables all of them, and the notebook rehearsal reports which paths ran on both GPUs. Estimated saving is roughly 1-1.5 h of the ~7 h run; the stage weights in `ber/progress.py` assume it.

**Kaggle vs AWS accuracy (26 Sep).** The Kaggle profile lowers the candidate ceiling. Measured on 18k train entities (same data): AWS blocking 0.99954, the first Kaggle setting (128-d) 0.99912, and the current one (192-d) 0.99935, with 40% fewer candidates than AWS (57 vs 95 per S1). The gap grows with dataset size (−0.00019 at 3k, −0.00042 at 18k entities), so it is expected to be larger at full scale; the real run's `train union` line reports it. Restoring the reverse-k or 256-d gave +0.00002 to +0.00006 for +21% candidates or ~25 GB RAM, so they were not restored. The cross-encoder swap (e5-small instead of bge-reranker-v2-m3) and the smaller GBDT training samples are not measured. Details: README §4.4.1.

**Speed calibration (26 Sep).** A Kaggle core is about 4x slower than a laptop core on the Python-heavy stages (measured from a real Kaggle log: mining pass A ~3.2k pairs/s/core vs ~13k locally). The stage-time weights in `ber/progress.py` were recalibrated from this; the full run is now estimated at about 7 h (range 5-9.5 h), not 5 h.
